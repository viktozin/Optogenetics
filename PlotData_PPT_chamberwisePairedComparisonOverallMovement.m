function PlotData_PPT_chamberwisePairedComparisonOverallMovement(root, resdir, NoDupesSafeData,NoDupesScaryData, mins, TypeOfTest, ScarySubstring, SafeSubstring)
...
b=bar(y);
   ...
   scatter(repmat(b(1).XEndPoints(2), length(InScarySafeCondChR2), 2),InScarySafeCondChR2,70,'MarkerFaceColor',[0.25, 0.55, 0.95],'MarkerEdgeColor','k','LineWidth',1);
   scatter(repmat(b(1).XEndPoints(1), length(InScarySafeCondGFP), 2),InScarySafeCondGFP,70,'MarkerFaceColor',[0.0, 0.6, 0.45],'MarkerEdgeColor','k','LineWidth',1);
...
elseif contains(VarStr, 'cent')||contains(VarStr, 'atio');
   ...
   % Legend=legend('Left chamber' ,'Right chamber' , 'Location','bestoutside', 'FontSize',13); %'Max, Border Zone', 'Mean, Centre', 'Max, Centre','Location','bestoutside');
...
ylabel('Overall movement, cm', FontSize=12);
   ...
   %subtitle(horzcat('Duration of trial: ',minsStr,' mins'),'FontSize',13);
    ...
    fnm2 = fullfile(resdir,'Average plots',strcat(Srepl,'_Paired_NOStim_OverallMovement',num2str(mins),'Mins.svg'));%fullfile(StatisticPlots, strcat(TypeOfTest,'_',ExperimentFolders{i},'_ZoneTransition_', nameFolds{h}, '.png'));
