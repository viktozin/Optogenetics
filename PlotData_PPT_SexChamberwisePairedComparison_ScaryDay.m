function PlotData_PPT_SexChamberwisePairedComparison_ScaryDay(root, resdir, NoDupesSafeData,NoDupesScaryData, mins, TypeOfTest, ScaryZoneSubstring, SafeZoneSubstring)
...
%for i=8:9 %8:length(NoDupesScaryData.Properties.VariableNames);
   ...
   % y=[mean(InScarySafeCondChR2) mean(InScarySafeCondGFP)];%[NoDupesScaryData.DurationInScaryZone NoDupesSafeData.DurationInScaryZone];
  %y=[mean(InScaryScaryCondChR2) mean(InScaryScaryCondGFP)];%[NoDupesScaryData.DurationInScaryZone NoDupesSafeData.DurationInScaryZone];
     y=[mean(InSafeScaryCondChR2) mean(InScaryScaryCondChR2); mean(InSafeScaryCondGFP) mean(InScaryScaryCondGFP)];
...
b=bar(y);
    ...
    if strcmp(CurrentSex,'Male');
        ...
        scatter(repmat(b(1).XEndPoints(1), length(InSafeScaryCondChR2), 1),InSafeScaryCondChR2,70,'MarkerFaceColor',[0.25, 0.55, 0.95],'MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(1).XEndPoints(2), length(InSafeScaryCondGFP), 1),InSafeScaryCondGFP,70,'MarkerFaceColor',[0.0, 0.6, 0.45],'MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(2).XEndPoints(1), length(InScaryScaryCondChR2), 2),InScaryScaryCondChR2,70,'MarkerFaceColor',[0.25, 0.55, 0.95],'MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(2).XEndPoints(2), length(InScaryScaryCondGFP), 2),InScaryScaryCondGFP,70,'MarkerFaceColor',[0.0, 0.6, 0.45],'MarkerEdgeColor','k','LineWidth',1);

        
    elseif contains(CurrentSex,'Female','IgnoreCase',true); %IF FEMALE

        
        scatter(repmat(b(1).XEndPoints(1), length(InSafeScaryCondChR2), 1),InSafeScaryCondChR2,70,'MarkerFaceColor','r','MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(1).XEndPoints(2), length(InSafeScaryCondGFP), 1),InSafeScaryCondGFP,70,'MarkerFaceColor',[0.0, 0.6, 0.45],'MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(2).XEndPoints(1), length(InScaryScaryCondChR2), 2),InScaryScaryCondChR2,70,'MarkerFaceColor','r','MarkerEdgeColor','k','LineWidth',1);
        scatter(repmat(b(2).XEndPoints(2), length(InScaryScaryCondGFP), 2),InScaryScaryCondGFP,70,'MarkerFaceColor',[0.0, 0.6, 0.45],'MarkerEdgeColor','k','LineWidth',1);
...
ylabel('Duration, s', FontSize=12);
   ...
   subtitle("Chamber stimulation condition");
...
Srepl="ChamberwiseComparison";

    fnm = fullfile(resdir,'Average plots',CurrentSex,strcat(Srepl,'_Paired_Stim',CurrentSex,num2str(mins),'Mins.png'));%fullfile(StatisticPlots, strcat(TypeOfTest,'_',ExperimentFolders{i},'_ZoneTransition_', nameFolds{h}, '.png'));
    fnm1 = fullfile(resdir,'Average plots',CurrentSex,strcat(Srepl,'_Paired_Stim',CurrentSex,num2str(mins),'Mins.fig'));%fullfile(StatisticPlots, strcat(TypeOfTest,'_',ExperimentFolders{i},'_ZoneTransition_', nameFolds{h}, '.png'));
    fnm2 = fullfile(resdir,'Average plots',CurrentSex,strcat(Srepl,'_Paired_Stim',CurrentSex,num2str(mins),'Mins.svg'));%fullfile(StatisticPlots, strcat(TypeOfTest,'_',ExperimentFolders{i},'_ZoneTransition_', nameFolds{h}, '.png'));
